...
function LernendeTable() {
    ...
    /* State für geladenen Lernendenen*/  
    ...
    /* Hier werden alle Lernendenen von der API geladen. Der API-Call wird asynchron ausgeführt */
    const getData = async () => {
    ...
    /* Hier wird der Lernenden gelöscht. Der API-Call wird asynchron ausgeführt */
    const deleteData = async () => {
        ...
        /* Fehler abfangen */
        try {
            const response = await axios.delete("https://nadia.dnet.ch/lernende/" + modalID);
            removeDataFromList();
    ...
    /* Der gelöschte Lernendenen wird aus dem state array entfernt*/
    ...
    function renderModal(){
        return(<Modal show={show} onHide={() => handleShow(false)}>
            <Modal.Header closeButton>
              <Modal.Title>Lernenden löschen</Modal.Title>
            </Modal.Header>
            <Modal.Body>Wollen Sie den ausgewählten Lernenden wirklich löschen?</Modal.Body>
            <Modal.Footer>
    ...
    return(
        <div>
            <h1>Lernenden Dashboard <Link className="btn btn-primary" to={`/lernende/add`}>Lernenden erfassen <PlusCircle color="white" size={15} /></Link></h1>
            <Alert show={showSuccess} variant="success">
                <p>
                 Lernenden wurde erfolgreich entfernt.
                </p>
            </Alert>
